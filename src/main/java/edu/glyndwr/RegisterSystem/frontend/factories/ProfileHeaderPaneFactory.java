...
public class ProfileHeaderPaneFactory {
    ...
    public GridPane buildNewPersonDataPane(RegisterSystemMainViewController controller) {
        ...
        controller.getStudentProfilBox().setOnAction(event -> {
            ...
            String courseString = controller.getModel().getCourseMemberList().stream()
        ...
        controller.getStudentCourseBox().setOnAction((Event event) -> {
            List<CourseDate> dates = controller.getModel().getCourseDateList().stream()
                    .filter(f -> f.getCourse().equals((Course) controller.getStudentCourseBox().getValue()))
                    .collect(Collectors.toList());
            dates.forEach(d -> controller.getModel().getAttendenceList().stream()
                    .filter(a -> a.getCourseDate().equals(d) && a.getCourseMember().getStudent().equals((Student) controller.getStudentProfilBox().getValue())));

            List<CourseDateAttendenceWrapper> wrappedAttndences = new ArrayList<>();
            dates.forEach((date) -> {
